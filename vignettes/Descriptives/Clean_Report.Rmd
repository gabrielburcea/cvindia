...
```{r symptoms_covid_in_resp, echo = F, results='asis',  fig.height=2, fig.width=7, cache=TRUE}
...
```{r symptom_profile_all_symptoms, echo = F, results='asis', fig.height=2, fig.width=7, cache=TRUE}
...
```{r symptom_profile_all_symptoms_numbers, echo= F, results='asis',   fig.height=2, fig.width=9, cache=TRUE}
...
```{r temperature, echo=FALSE, results='asis', fig.height=2, fig.width=7, cache=TRUE}
...
```{r mild_symptom, echo=FALSE, results='asis', fig.height=2, fig.width=7, cache=TRUE}
...
```{r moderate,echo=FALSE, results='asis', fig.height=2, fig.width=5, cache=TRUE }
...
data_select <- read_csv("/Users/gabrielburcea/Rprojects/data/PivotMappe060520r.csv")
...
cov_sympt_plot <- cvindia::no_cov(data = data_no_sev,start_date = as.Date("2020-04-09", tz = "Europe/London"),
                              end_date = as.Date("2020-05-06",tz = "Europe/London"), plot_chart = TRUE)
...
hypertension_num <- comorb_cov_sympt_num %>%
  dplyr::filter(Morbidity == "High Blood Pressure (hypertension)") %>%
...
plotly::ggplotly(plot_hypertension_sympt)
...
plot_obesity_sympt <-
    ggplot2::ggplot(obesity_num, ggplot2::aes(x = Symptom, y = Count, fill = Count)) +
    ggplot2::coord_flip() +
    ggplot2::geom_bar(stat = "identity", position = "dodge") +
    ggplot2::scale_fill_viridis_c(option = "magma", direction = -1) +
...
plotly::ggplotly(plot_obesity_sympt)

```

```{r, asthma, echo=FALSE, results='asis', fig.height=5, fig.width=9, cache=TRUE}
asthma_num <- comorb_cov_sympt_num %>%
  dplyr::filter(Morbidity == "Asthma (managed with an inhaler)") %>%
  dplyr::arrange(desc(Count))
...
plotly::ggplotly(plot_asthma_sympt)


```


```{r, lung, echo=FALSE, results='asis', fig.height=5, fig.width=9, cache=TRUE }
lung_num <- comorb_cov_sympt_num %>%
  dplyr::filter(Morbidity == "Long-Standing Lung Condition") %>%
  dplyr::arrange(desc(Count))
...
plotly::ggplotly(plot_lung_sympt)
```

```{r}
heart_num <- comorb_cov_sympt_num %>%
  dplyr::filter(Morbidity == "Long-Standing Heart Disease") %>%
  dplyr::arrange(desc(Count))


title_stub <- "Long-Standing Heart Disease across symptoms\n"
start_date_title <- format(as.Date(start_date), format = "%d %B %Y")
end_date_title <- format(as.Date(end_date), format = "%d %B %Y")
chart_title <- paste0(title_stub, start_date_title, " to ", end_date_title)
...
plotly::ggplotly(plot_heart_sympt)
...
kidney_num <- comorb_cov_sympt_num %>%
  dplyr::filter(Morbidity == "Long-Standing Kidney Disease") %>%
  dplyr::arrange(desc(Count))
...
plotly::ggplotly(plot_kidney_sympt)
...
liver_num <- comorb_cov_sympt_num %>%
  dplyr::filter(Morbidity == "Long-Standing Liver Disease") %>%
  dplyr::arrange(desc(Count))
...
plotly::ggplotly(plot_liver_num)
```


```{r}
diab_type_one_num <- comorb_cov_sympt_num %>%
  dplyr::filter(Morbidity == "Diabetes Type 1 (controlled by insulin)") %>%
  dplyr::arrange(desc(Count))
...
plot_diab_1_num <-
    ggplot2::ggplot(diab_type_one_num, ggplot2::aes(x = Symptom, y = Count, fill = Count)) +
    ggplot2::coord_flip() +
    ggplot2::geom_bar(stat = "identity", position = "dodge") +
    ggplot2::scale_fill_viridis_c(option = "magma", direction = -1) +
    ggplot2::scale_x_discrete(limits = unique(diab_type_one_num$Symptom)) +
    #ggplot2::theme(legend.position = "bottom") +
    #ggplot2::guides(fill = ggplot2::guide_legend(nrow = 3)) +
    ggplot2::theme_minimal() +
    ggplot2::labs( title = chart_title,
...
plotly::ggplotly(plot_diab_1_num)
```

```{r}
	
diab_type_two_num <- comorb_cov_sympt_num %>%
...
plot_diab_2_num <-
    ggplot2::ggplot(diab_type_two_num, ggplot2::aes(x = Symptom, y = Count, fill = Count)) +
    ggplot2::coord_flip() +
    ggplot2::geom_bar(stat = "identity", position = "dodge") +
    ggplot2::scale_fill_viridis_c(option = "magma", direction = -1) +
    ggplot2::scale_x_discrete(limits = unique(diab_type_two_num$Symptom)) +
    #ggplot2::theme(legend.position = "bottom") +
    #ggplot2::guides(fill = ggplot2::guide_legend(nrow = 3)) +
    ggplot2::theme_minimal() +
    ggplot2::labs( title = chart_title,
...
plotly::ggplotly(plot_diab_2_num)
...
show_symp_num <- knitr::kable(dplyr::top_n(show_symp_num, 10))
...
```{r}
data_categ_nosev <- readr::read_csv("/Users/gabrielburcea/Rprojects/stats_data_whole/data_categ_no_sev.csv")
...
data_categ_nosev <- data_categ_nosev %>% dplyr::rename(id = ID)
...
```{r}
obesity_model <- glm(obesity ~ cough + diarrhoea + shortness_breath + temperature, data = obesity_data, family = binomial)
...
exp(cbind(OR = coef(obesity_model), confint(obesity_model)))
...
asthma_data <- data_categ_nosev %>%
  dplyr::select(asthma, diabetes_type_one, diabetes_type_two, obesity, hypertension, heart_disease, lung_condition, 
                ...
                nasal_congestion, nausea_vomiting, shortness_breath, sore_throat, sputum, temperature, covid_tested) %>%
  tidyr::drop_na()
...
asthma_covid <- asthma_data %>%
  dplyr::select(asthma, covid_tested) %>%
  dplyr::group_by(asthma, covid_tested) %>%
  dplyr::tally()
...
exp(cbind(OR = coef(asthma_model), confint(asthma_model)))
```


```{r}
diabetes_type_one_data <- data_categ_nosev %>%
  dplyr::select(diabetes_type_one, gender,  chills, cough, diarrhoea, headache, loss_smell_taste, muscle_ache, 
                nasal_congestion, nausea_vomiting, shortness_breath, sore_throat, sputum, temperature) %>%
  tidyr::drop_na()
```
...
diabetes_type_one_model <- glm(diabetes_type_one ~ chills + cough + headache + loss_smell_taste + muscle_ache + nausea_vomiting +
                     temperature, data = diabetes_type_one_data, family = binomial)
...
diabetes_type_two_data <- data_categ_nosev %>%
  dplyr::select(diabetes_type_two, gender, age, chills, cough, diarrhoea, headache, loss_smell_taste, muscle_ache, 
                nasal_congestion, nausea_vomiting, shortness_breath, sore_throat, sputum, temperature) %>%
  tidyr::drop_na()
```
...
heart_data <- data_categ_nosev %>%
  dplyr::select(heart_disease, gender, age, chills, cough, diarrhoea, headache, loss_smell_taste, muscle_ache, 
                nasal_congestion, nausea_vomiting, shortness_breath, sore_throat, sputum, temperature) %>%
  tidyr::drop_na()
```
...
liver_data <- data_categ_nosev %>%
  dplyr::select(liver_disease, gender, age, chills, cough, diarrhoea, headache, loss_smell_taste, muscle_ache, 
                nasal_congestion, nausea_vomiting, shortness_breath, sore_throat, sputum, temperature) %>%
  tidyr::drop_na()
```
...
lung_condition_data <- data_categ_nosev %>%
  dplyr::select(lung_condition, gender, age, chills, cough, diarrhoea, headache, loss_smell_taste, muscle_ache, 
                nasal_congestion, nausea_vomiting, shortness_breath, sore_throat, sputum, temperature) %>%
  tidyr::drop_na()
```
...
kidney_data <- data_categ_nosev %>%
  dplyr::select(kidney_disease, gender, age,  chills, cough, diarrhoea, headache, loss_smell_taste, muscle_ache, 
                nasal_congestion, nausea_vomiting, shortness_breath, sore_throat, sputum, temperature) %>%
  tidyr::drop_na()
```
