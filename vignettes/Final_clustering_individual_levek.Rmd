...
gower_distance <- cluster::daisy(data_test[,2:22], metric = "gower")
...
```{r}
# Compute with agnes 
hc_agnes <- agnes(data_test[,2:22], method = "complete")
...
elb_agglomerative <-
  ggplot(data = data.frame(t(
    cstats.table(gower_distance, hc_agnes, 7)
  )),
  aes(x = cluster.number, y = within.cluster.ss)) +
  geom_point() +
  geom_line() +
