...
map <- ggplot2::ggplot(data = map.world_joined) +
  ...
  ggplot2::labs(title = "World map: Count of responders per country",
                y = "lat", x = "long") +
  theme_minimal() +
  ggplot2::theme(
...
plot_countries <- ggplot2::ggplot(world_map_n, aes(x = reorder(Country, -Percentage), y = Percentage)) + 
  ggplot2::geom_bar(stat = "identity", fill = "steelblue", width = 0.9) +
  coord_flip() + 
  theme_minimal() + 
  ggplot2::labs(title = "% of responders across countries in Your.md dataset",
                y = "Percentage", x = "Countries", caption = "Source: Your.md") +
