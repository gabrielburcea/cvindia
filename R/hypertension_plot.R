...
#'
#' @examples
hypertension_plot <- function(data = data, start_date = as.Date("2020-04-09", tz = "Europe/London"), end_date = as.Date("2020-08-20"), 
                              plot_chart = TRUE) {
  ...
  hypertension_num <- comorb_cov_sympt_num %>%
    dplyr::filter(Morbidity == "Hypertension") %>%
  ...
  plot_hypertension_sympt <-
    ggplot2::ggplot(hypertension_num, ggplot2::aes(x = Symptom, y = Count, fill = Count)) +
    ggplot2::coord_flip() +
    ggplot2::geom_bar(stat = "identity", position = "dodge") +
    ggplot2::scale_fill_viridis_c(option = "magma", direction = -1) +
    ggplot2::scale_x_discrete(limits = unique(hypertension_num$Symptom)) +
