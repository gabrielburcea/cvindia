...
#'
#' @examples
map_severe_prof <- function(data, item = "world", start_date = as.Date("2020-04-01", format = "%Y-%m-%d"),
  ...
  # Set the title
  title_stub <- "Count of respondents with severe symptom per country,  "
  start_date_title <- format(as.Date(start_date), format = "%d %B %Y")
  end_date_title <- format(as.Date(end_date), format = "%d %B %Y")
  chart_title <- paste0(title_stub, start_date_title, " to ", end_date_title)
  
  
  map_severe <- ggplot2::ggplot() +
    ggplot2::geom_polygon(data =  map.world_joined_severe, ggplot2::aes(
      x = long,
      y = lat,
      group = group,
      fill = Count
    )) +
    ggplot2::geom_path(data =  map.world_joined_severe, ggplot2::aes(
      x = long,
      y = lat,
      group = group),
  ...
  if(plot_chart ==TRUE){
    ...
    count_respondents_severe_n <- count_respondents_severe %>%
      dplyr::select(Country, Self_diagnosis, Count, Frequency) %>%
      dplyr::arrange(dplyr::desc(Count)) %>%
      dplyr::top_n(20)
    
    count_respondents_severe_n 
  }
