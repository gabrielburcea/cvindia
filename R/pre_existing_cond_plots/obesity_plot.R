...
#'
#' @examples
obesity_plot <- function(data = data, start_date = as.Date("2020-04-09", tz = "Europe/London"), end_date = as.Date("2020-08-20"), plot_chart = TRUE){
  
  comorb_cov_sympt_num <- cvindia::comorbidities_symptoms(data, start_date,  end_date,  plot_chart = FALSE)
  
  obesity_num <- comorb_cov_sympt_num %>%
    dplyr::filter(Morbidity == "Obesity") %>%
  ...
  plot_obesity_sympt <-
    ggplot2::ggplot(obesity_num, ggplot2::aes(x = reorder(Symptom, - Percentage), y = Count, fill = Count)) +
    ggplot2::coord_flip() +
    ggplot2::geom_bar(stat = "identity", position = "dodge") +
    ggplot2::scale_fill_viridis_c(option = "magma", direction = -1) +
    ggplot2::scale_x_discrete(limits = unique(obesity_num$Symptom)) +
