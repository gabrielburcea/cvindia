...
#'
#' @examples
mild_comorbid_covsympt <- function(data, start_date = as.Date("2020-04-09", tz = "Europe/London"),
  ...
  plot_comorb_cov_sympt_mild <- ggplot2::ggplot(gather_divided_mild,  ggplot2::aes(Morbidity, Count, group = Symptom)) + #shape = Event,  colour = Event
    ...
    ggplot2::labs(title = chart_title,
                  subtitle = "Counts of SARS-Covid-19 symptoms by comorbidities for patients with mild version of virus",
  ...
  if(plot_chart == TRUE){
    ...
    gather_divided_mild_numbers <- gather_divided_mild %>%
      dplyr::arrange(desc(Count)) %>%
      dplyr::top_n(10)
    gather_divided_mild_numbers
  }
}
