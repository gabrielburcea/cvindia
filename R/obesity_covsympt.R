...
#'
#' @examples
obesity_covsympt <- function(data, start_date = as.Date("2020-04-09", tz = "Europe/London"),
  ...
  gather_divided_obesity <- comorb_divided %>%
    ...
    tidyr::pivot_longer(cols=starts_with("Comorbidity"), 
                        ...
                        values_to="Morbidity") %>%
    dplyr::filter(Morbidity != "None" & Morbidity == "Obesity") %>%
  ...
  # Set the title
  title_stub <- "Obese patients and Covid symptoms\n"
  ...
  plot_comorb_cov_sympt_severe <- ggplot2::ggplot( gather_divided_asthma, ggplot2::aes(x = reorder(Symptom, -Count), Count, fill = Severity)) +
    ...
    ggplot2::labs(title = chart_title,
                  subtitle = "Counts of obese patients and severity of Covid symptoms",
  ...
  if(plot_chart == TRUE){
    ...
    gather_divided_obesity_numbers <- gather_divided_obesity %>%
      dplyr::arrange(desc(Count)) %>%
      dplyr::top_n(10)
    
    gather_divided_obesity_numbers
