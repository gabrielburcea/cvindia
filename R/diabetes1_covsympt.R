...
#'
#' @examples
diabetes1_covsympt <- function(data, start_date = as.Date("2020-04-09", tz = "Europe/London"),
  ...
  gather_divided_diabetes1 <- comorb_divided %>%
    ...
    tidyr::pivot_longer(cols=starts_with("Comorbidity"), 
                        ...
                        values_to="Morbidity") %>%
    dplyr::filter(Morbidity != "None" & Morbidity == "Diabetes Type 1 (controlled by insulin)") %>%
  ...
  # Set the title
  title_stub <- "Patients with Diabetes Type 1 and Covid symptoms\n"
  start_date_title <- format(as.Date(start_date), format = "%d %B %Y")
  end_date_title <- format(as.Date(end_date), format = "%d %B %Y")
  chart_title <- paste0(title_stub, start_date_title, " to ", end_date_title)
  
  
  plot_diabetes1 <- ggplot2::ggplot( gather_divided_diabetes1, ggplot2::aes(x = reorder(Symptom, -Count), Count, fill = Severity)) +
    ggplot2::geom_col(ggplot2::aes(colour = Severity)) +
    ggplot2::coord_flip() + 
    ggplot2::scale_fill_brewer(palette = 'RdPu')  +
    ggplot2::theme_bw() +
    ggplot2::labs(title = chart_title,
                  subtitle = "Counts of patients with Diabetes Type 1 and severity of Covid symptoms",
  ...
  if(plot_chart == TRUE){
    ...
    gather_divided_diabetes1_numbers <- gather_divided_diabetes1 %>%
      dplyr::arrange(desc(Count)) %>%
      dplyr::top_n(20)
    
    gather_divided_diabetes1_numbers
