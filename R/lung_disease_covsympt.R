...
#'
#' @examples
lung_disease_covsympt <- function(data, start_date = as.Date("2020-04-09", tz = "Europe/London"),
  ...
  gather_divided_lung_disease <- comorb_divided %>%
    ...
    tidyr::pivot_longer(cols=starts_with("Comorbidity"), 
                        ...
                        values_to="Morbidity") %>%
    dplyr::filter(Morbidity != "None" & Morbidity == "Long-Standing Lung Condition") %>%
  ...
  # Set the title
  title_stub <- "Patients with lung disease and Covid symptoms\n"
  start_date_title <- format(as.Date(start_date), format = "%d %B %Y")
  end_date_title <- format(as.Date(end_date), format = "%d %B %Y")
  chart_title <- paste0(title_stub, start_date_title, " to ", end_date_title)
  
  
  plot_lung_disease <- ggplot2::ggplot(gather_divided_lung_disease, ggplot2::aes(x = reorder(Symptom, -Count), Count, fill = Severity)) +
    ggplot2::geom_col(ggplot2::aes(colour = Severity)) +
    ggplot2::coord_flip() + 
    ggplot2::scale_fill_brewer(palette = 'PuBu')  +
    ggplot2::theme_bw() +
    ggplot2::labs(title = chart_title,
                  subtitle = "Counts of patients with long-standing lung disease and severity of Covid symptoms",
  ...
  if(plot_chart == TRUE){
    ...
    gather_divided_lung_disease_numbers <- gather_divided_lung_disease %>%
      dplyr::arrange(desc(Count)) %>%
      dplyr::top_n(10)
    
    gather_divided_lung_disease_numbers
