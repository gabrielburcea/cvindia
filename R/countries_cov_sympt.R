...
#'
#' @examples
countries_cov_sympt <- function(data, start_date = as.Date("2020-04-09", tz = "Europe/London"),
                                end_date = as.Date("2020-05-06",tz = "Europe/London"), plot_chart = TRUE){
  
  levels <- c('United Kingdom' = "Great Britain", 'United Kingdom' = "England")
  
  comorb_divided <- data %>%
    dplyr::mutate(country = forcats::fct_recode(fct_recode(Country, !!!levels))) %>%
  ...
  title_stub <- "Symptom mapping accross per country\n"
  ...
  plot_comorb_cov_sympt <-
    ...
    ggplot2::labs( title = chart_title,
                   subtitle = "Percentages: Symptom mapping accross the five highest percentage of respondents per country",
                   y = "Percentages",
                   x = "Country",
